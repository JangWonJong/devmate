...
export function NewPostPage() {
  ...
  const [type, setType] = useState<'QUESTION' | 'STUDY'>(
    state?.prefilledType ?? 'QUESTION'
  )
  ...
  const isFromDevLog =
    Boolean(state?.prefilledTitle) || Boolean(state?.prefilledContent)
      ...
      <section className="space-y-3">
        ...
        {isFromDevLog && (
          <div className="rounded-2xl border border-blue-100 bg-blue-50 px-4 py-3 text-sm text-blue-700">
            DevLog 내용을 바탕으로 게시글 초안이 자동 입력되었어요. 필요한
            부분만 다듬어서 등록해 주세요.
          </div>
        )}
      </section>
          ...
          <div className="space-y-3">
            ...
            >
              ...
              <p className="mt-2 text-xs text-slate-500">
                한 번에 여러 장 선택하거나, 여러 번 나눠서 추가할 수 있어요.
                최대 5장, 각 5MB까지 업로드 가능해요.
              </p>
            </div>
          </div>
