...
import { KakaoPlaceSearchModal, type KakaoPlace } from "../../common/map/KakaoPlaceSearchModal"

type StudyCreateModalProps = {
  open: boolean
  title: string
  loading?: boolean
  showPlaceFields?: boolean
  ...
  onChangeMaxMembers: (value: string) => void
...
export function StudyCreateModal({
  open,
  title,
  loading = false,
  showPlaceFields = true,
        ...
        <div className="mt-5 space-y-4">
          <div className="space-y-2">
            ...
            <input
              ...
              min={2}
              value={maxMembers}
              onChange={(e) => onChangeMaxMembers(e.target.value)}
              placeholder="예: 4"
            ...
            <>
              <div className="space-y-2">
                ...
                <textarea
                  rows={4}
                  value={notice}
                  onChange={(e) => onChangeNotice(e.target.value)}
                  placeholder="예: 첫 모임 일정이나 준비물을 적어주세요."
                  className="w-full resize-none rounded-2xl border border-slate-300 px-4 py-3 text-sm outline-none transition focus:border-indigo-500"
                />
              </div>
