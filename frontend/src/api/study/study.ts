...
export type StudyPlaceUpdateRequest = {
  placeName?: string
  address?: string
  latitude?: number
  longitude?: number
...
export async function getStudyMembers(studyId: number) {
  const { data } = await http.get<ApiResponse<StudyMemberResponse[]>>(
...
export async function getMyStudies() {
  const { data } =
    await http.get<ApiResponse<StudyResponse[]>>('/api/studies/me')
  if (!data.success || data.data == null) {
...
export async function updateStudyCapacity(
  studyId: number,
  req: { maxMembers: number }
) {
  const { data } = await http.patch<ApiResponse<number>>(
    `/api/studies/${studyId}/capacity`,
    req
  )
...
export async function updateStudyPlace(
  studyId: number,
  request: StudyPlaceUpdateRequest
) {
  const { data } = await http.patch<ApiResponse<number>>(
    `/api/studies/${studyId}/place`,
    request
  )

  if (!data.success || data.data == null) {
    throw new Error('스터디 장소 수정 실패')
  }
